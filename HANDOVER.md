...
- **Foundation tone/presence retry:** `scripts\train_profile.py` / `src\sonna_editor\training\profile_runner.py` now support repeatable `--field-loss-weight FIELD=WEIGHT` overrides for any configured Lightroom slider, and `scripts\train_foundation_model.py` passes them through. Use this as the quick retry path when a foundation run is close on WB but misses tone/presence metrics such as Whites, Blacks, Highlights, Vibrance, or Saturation. This creates a fresh run from the same prepared splits, warm-starting weights from the active foundation by default; `--resume-from-checkpoint` remains only for interrupted-run continuation.
