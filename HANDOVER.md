...
- *Initial state* = a Lightroom preset (baseline slider values) + a style survey (user preferences over the 135-slider space)
- A **preset-to-checkpoint converter** (new build piece — Mode B rebuild step b) ingests the preset + survey and produces a `SonnaEditor` ckpt:
  ...
  - Backbone warm-loaded from the current Mode A production checkpoint (v1.2.3 today) so image features are useful from epoch zero
...
- After initialisation, Mode B *is* a model — same architecture, same 135-slider output, same `inference/engine.py` and `inference/pipeline.py` as Mode A
...
17. **Mode B rebuild track — COMPLETE 2026-05-14 (supersedes Phase 2's rule-based Mode B as the production direction):** Mode B is now a model-based profile type that shares the `SonnaEditor` architecture and Phase 5 fine-tuning pipeline with Mode A — only the initial checkpoint differs (see Decision 4 for full rationale). All three substeps shipped:
    ...
    - **Step 2: Preset-to-checkpoint converter. DONE 2026-05-14 (`a8d7c04`).** `scripts/build_mode_b_checkpoint.py` ingests `(preset.xmp + survey.json)` and produces a `SonnaEditor` ckpt. Implementation in `src/sonna_editor/mode_b/checkpoint_builder.py`: loads v1.2.3 backbone + metadata encoder weights, computes per-slider bias in prediction space (log-K for Temperature, raw units elsewhere; preset value + survey offset, clamped to LR range), zeros each head's final-layer weights and sets biases via `apply_biases_to_model`, saves as a v1 SonnaEditor ckpt with sidecar JSON marking `profile_type: "mode_b_initial"`. Includes built-in verification step (forward pass on dummy image; output must match bias targets within 1e-3 absolute / 1e-2 relative for Temperature). 29 tests in `tests/test_checkpoint_builder.py`.
...
Mode B profiles are model-based ckpts produced from a Lightroom preset + style-survey JSON, warm-loaded from a Mode A trained backbone (currently v1.2.3). The inference and Phase 5 fine-tuning paths are shared with Mode A — there's no separate "Mode B engine". See Decision 4 + Part 6 item 17 for the architectural reasoning.

**Three-step workflow:**

1. **Generate a style survey JSON** (one-time per profile). Captures the user's preferences across six high-variation sliders (Exposure, Temperature, Tint, Contrast, Saturation, Shadows) as integer answers in `{-2, -1, 0, +1, +2}`. Two modes:
...
2. **Build the Mode B initial checkpoint** from a Lightroom preset + the survey:
   ...
   Produces `mode_b.ckpt` (the model, ~118 MB) + `mode_b.json` (sidecar with `profile_type: "mode_b_initial"`, `profile_id`, `base_checkpoint`, `slider_set_version: "v1"`, `default_skip_fields` inherited from v1.2.3, and `resolution` inherited from the base ckpt). Built-in verification step confirms the saved ckpt's forward pass matches the preset+survey targets within 1e-3 abs / 1e-2 relative for Temperature.
