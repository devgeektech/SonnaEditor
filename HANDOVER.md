...
- **Training runner structure:** the production training callable lives in `src/sonna_editor/training/profile_runner.py`; `scripts/train_profile.py` is a thin CLI wrapper. The API uses the packaged runner for frontend Personal AI training jobs, resolving the hidden foundation checkpoint as the warm start, with epoch progress, cancellation, and no publish on cancel. Personal AI sidecars now record foundation provenance (`foundation_version`, checkpoint path, SHA256, type, capabilities, and source tags), image-to-image foundation warm starts initialise slider output heads from RAW+XMP target medians to avoid random-head noise, and frontend/foundation warm-start runs use the progressive backbone schedule by default.
