...
def estimate_straighten_angle(image: Image.Image) -> StraightenResult:
    ...
    concentration = max(_axis_concentration(residual, weights), projection_confidence)
    angle = projection_angle if projection_confidence >= 0.05 else -float(median_residual)
    angle = max(-_MAX_APPLY_ANGLE, min(_MAX_APPLY_ANGLE, angle))

    abs_angle = abs(angle)
    if abs_angle < _MIN_APPLY_ANGLE:
        return StraightenResult(0.0, concentration, False, "angle_too_small", edge_count)
    if concentration < _MIN_CONFIDENCE:
        return StraightenResult(angle, concentration, False, "low_confidence", edge_count)

    return StraightenResult(round(angle, 4), concentration, True, "applied", edge_count)
