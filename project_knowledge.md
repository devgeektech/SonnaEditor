...
- Tone/presence focused loss overrides, 2026-06-10: `src/sonna_editor/training/profile_runner.py` accepts repeatable `--field-loss-weight FIELD=WEIGHT` overrides for any slider in `config.SLIDER_FIELDS`, records the parsed map in `training_summary.json`, and `scripts/train_foundation_model.py` passes those overrides through. Use this before collecting more data when a foundation run is close on WB but misses tone/presence gate metrics such as Whites, Blacks, Highlights, Vibrance, or Saturation.
