...
Current local workspace state as of 2026-06-05:
...
- Foundation warm-start retention, 2026-06-04 and simplification/capacity update 2026-06-05: frontend Personal AI uses the progressive backbone schedule by default. Foundation training now starts with the final ConvNeXt stage trainable (`stage:7`) plus feature fusion/heads, then progressive unfreeze can expand later. Compact specs such as `block:7:2,stage:6`, `block:7:1-2,stage:6`, `stage:7`, and `from:6` are supported for ablations. Warm starts now come from native `SonnaEditor` slider-regression checkpoints only. Use `scripts/analyse_backbone_drift.py` after training to quantify whether foundation features were preserved.
