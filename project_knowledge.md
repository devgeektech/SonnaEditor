...
Current local workspace state as of 2026-06-03:
...
- There is no guaranteed local `v1_learning/dataset/` split set or frontend-visible `v1_learning/model-v*.ckpt` profile until fresh RAW+XMP data is added and a Personal AI profile is trained or a checkpoint is intentionally published.
...
- `data/`: gitignored local learning area. `data/training_sources/` stores source RAW/XMP inputs in separate child folders per dataset or run, plus future FiveK image-pair folders such as `raw_dng/` and `expert_tiff/`; `data/training_workspace/` stores generated datasets and training runs. Fresh clones auto-create this tree at runtime.
...
| `src/sonna_editor/training/image_foundation.py` | Image-to-image foundation training path for paired `RAW/DNG/image -> edited TIFF` data. Matches pairs by stem, trains a ConvNeXt encoder plus lightweight decoder with L1 + SSIM loss, and saves a hybrid foundation `model.ckpt` that keeps `SonnaEditor` as the canonical slider checkpoint while storing the image decoder as auxiliary state. |
...
- FiveK TIFF foundation path, 2026-06-03 and hybrid update 2026-06-04: MIT-Adobe FiveK should not be forced into the current RAW+XMP pipeline. FiveK provides 5,000 DNG inputs and five expert TIFF renditions, not direct XMP labels. `src/sonna_editor/training/image_foundation.py` implements paired image training from DNG/image inputs to expert TIFF targets and now saves a hybrid foundation checkpoint: native `SonnaEditor` slider state remains loadable for Personal AI/Lite, and the image decoder is stored as auxiliary state for future TIFF continuation. Only investigate `fivek.lrcat` slider extraction as a separate research path.
...
- Hybrid foundation checkpoint, 2026-06-04: TIFF foundation training now promotes one checkpoint that can be used by both future TIFF and RAW+XMP foundation runs. The checkpoint's `model_state` remains a native `SonnaEditor` state dict for Personal AI/Lite compatibility, while `image_decoder_state` stores the image reconstruction decoder. RAW+XMP foundation training preserves `image_decoder_state`; TIFF training preserves existing slider heads and marks whether those heads have actually been RAW+XMP trained.
