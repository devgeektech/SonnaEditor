...
- Image-to-image foundation warm starts now initialise SonnaEditor slider output heads from RAW+XMP training target medians, preventing random slider heads from injecting noisy gradients into the transferred backbone at the start of Personal AI training. Normal SonnaEditor checkpoint warm starts keep their trained heads unchanged.
...
- Foundation TIFF training now writes a hybrid single checkpoint as `model.ckpt`: the canonical payload is still a loadable `SonnaEditor` slider-regression checkpoint, and the image decoder is stored as auxiliary `image_decoder_state`. RAW+XMP foundation training carries that decoder forward, so the active foundation lineage can alternate RAW+XMP and RAW/DNG->TIFF runs without discarding the other training source.
